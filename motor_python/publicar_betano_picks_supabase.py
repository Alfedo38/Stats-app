...
def get_database_url(cli_value: str | None = None) -> str:
    ...
    if url.endswith(".db"):
        raise RuntimeError("Recibí un archivo .db, pero este publicador ahora trabaja con Postgres/Supabase.")
...
def connect_pg(args: argparse.Namespace):
    effective = getattr(args, "db_url", None) or (getattr(args, "db", None) if getattr(args, "db", None) and not str(args.db).endswith(".db") else None)
    return psycopg2.connect(get_database_url(effective), cursor_factory=RealDictCursor)


def latest_picks_snapshot(con, table: str) -> Optional[str]:
    with con.cursor() as cur:
        cur.execute(
            ...
            ORDER BY max_created DESC NULLS LAST, snapshot_id DESC
            LIMIT 1
            """
        )
        row = cur.fetchone()
...
def load_picks(args: argparse.Namespace) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if not snapshot_id:
            snapshot_id = latest_picks_snapshot(con, args.picks_table) or ""

        if not snapshot_id:
            print("⚠️ No encontré snapshot_id en la tabla Postgres de picks.")
        ...
        with con.cursor() as cur:
            cur.execute(
                f"""
                SELECT *
                FROM {qname(args.picks_table)}
                WHERE snapshot_id = %s
                  AND CAST(odds_decimal AS NUMERIC) >= %s
                  AND CAST(odds_decimal AS NUMERIC) <= %s
                ORDER BY CAST(ev_pct AS NUMERIC) DESC
                LIMIT %s
                """,
                (snapshot_id, args.min_odds, args.max_odds, sql_limit),
            )
...
def ensure_publish_table(con, table: str) -> None:
    with con.cursor() as cur:
        cur.execute(f"""
...
def publish_to_supabase(blocks: List[Dict[str, Any]], args: argparse.Namespace) -> None:
    con = connect_pg(args)
    try:
        ensure_publish_table(con, args.supabase_table)
        with con.cursor() as cur:
            if args.delete_pending_date:
                cur.execute(
                    f"DELETE FROM {qname(args.supabase_table)} WHERE pick_date = %s AND status = 'PENDING'",
                    (args.pick_date,),
                )
                print(f"🧹 Borrado previo PENDING {args.pick_date}: {cur.rowcount} fila(s)")

            cur.execute(
                f"""
                INSERT INTO {qname(args.supabase_table)} (json_data, results_data, pick_date, status)
                VALUES (%s, %s, %s, %s)
                """,
                (Json(blocks), None, args.pick_date, "PENDING"),
            )
        con.commit()
    except Exception:
        con.rollback()
    ...
    print(f"✅ Insert Postgres/Supabase: filas enviadas=1 | blocks={len(blocks)} | tabla={args.supabase_table}")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Publicar Betano Hitos como combinadas X2/X5/X10/X20 en Supabase/Postgres.")
    p.add_argument("--db-url", default=None, help="URL Postgres/Supabase. Si no se pasa, usa .env.local.")
    p.add_argument("--db", default=None, help="Compatibilidad: ignorado salvo que sea una URL Postgres. Ya no se usa ludo.db.")
    p.add_argument("--picks-table", default="public.picks_betano_hitos")
    p.add_argument("--sqlite-table", default=None, help="Compatibilidad antigua; si se pasa, se ignora salvo para mapear a --picks-table.")
    p.add_argument("--supabase-table", default="public.betano_picks")
    ...
    p.add_argument("--supabase-url", default=None, help="Compatibilidad antigua REST; ya no se usa.")
    p.add_argument("--supabase-key", default=None, help="Compatibilidad antigua REST; ya no se usa.")
    p.add_argument("--snapshot-id", default=None, help="Snapshot específico de picks_betano_hitos. Si no se pasa, usa el último snapshot Postgres.")
    ...
    args = p.parse_args()
    if args.sqlite_table and not args.picks_table:
        args.picks_table = args.sqlite_table
    return args
...
def main() -> None:
    ...
    print("========================================================================")
    print("🚀 PUBLICAR BETANO PICKS → SUPABASE/POSTGRES COMBOS")
    print("========================================================================")
    print("DB             : Postgres/Supabase")
    print(f"Tabla picks    : {args.picks_table}")
    print(f"Tabla destino  : {args.supabase_table}")
    ...
    print(f"Snapshot PG    : {getattr(args, 'resolved_snapshot_id', args.snapshot_id or 'AUTO')}")
