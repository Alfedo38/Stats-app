...
OUTPUT_CSV = Path("ludo_smoke_predictions.csv")
...
def print_top(df: pd.DataFrame, top: int):
    ...
    cand = cand.sort_values(["edge", "diff_abs"], ascending=False).head(top)

    print(f"\n🎯 Top {min(top, len(cand))} candidatos básicos:")
    display = cand[cols].copy()

    for c in ["line", "price", "proj", "diff", "edge", "model_mae", "min_L5"]:
...
def main():
    parser = argparse.ArgumentParser(description="LUDO v2 smoke test fix")
    parser.add_argument("--top", type=int, default=30, help="Cantidad de candidatos a mostrar")
    args = parser.parse_args()

    print("🧪 LUDO v2 SMOKE TEST FIX")
    print("=" * 72)
    ...
    print("\nNota: esto NO es todavía el motor final de picks. Es una prueba de compatibilidad DB + modelos.")
