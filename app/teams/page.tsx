        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {NBA_TEAMS.map((team) => (
            <Link
              key={team.id}
              href={`/teams/${team.id}`}
              className={`bg-[var(--surface)] border border-[var(--border)] p-4 rounded-3xl flex flex-col items-center justify-center gap-4 transition-all duration-300 group ${team.color} hover:bg-[var(--surface-hover)] hover:-translate-y-1 hover:shadow-2xl hover:shadow-slate-200 dark:hover:shadow-black`}
            >

              {/* El Escudo CSS Minimalista */}
              <div className="w-16 h-16 rounded-full border-2 border-[var(--border)] bg-[var(--surface-soft)] flex items-center justify-center group-hover:border-[var(--border-strong)] transition-colors relative overflow-hidden">
                ...
                <span className={`font-black text-2xl tracking-tighter text-[var(--text-muted)] transition-colors duration-300 ${team.glow}`}>
                  {team.id}
                </span>
              </div>
              ...
              <div className="text-center">
                <h2 className="text-[var(--text)] font-black text-[11px] uppercase tracking-tighter">{team.name}</h2>
                <p className="text-[var(--text-muted)] text-[9px] font-bold uppercase tracking-widest mt-1 group-hover:text-[var(--text)] transition-colors flex items-center justify-center">
