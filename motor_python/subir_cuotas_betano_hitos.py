...
Lee el CSV normalizado de Betano hitos y lo sube a Postgres/Supabase.

Usos:
  python3 subir_cuotas_betano_hitos.py --dry-run
  python3 subir_cuotas_betano_hitos.py
  python3 subir_cuotas_betano_hitos.py --input cuotas_procesadas/betano_hitos_20260512_130723.csv
...
def get_database_url(cli_value: str | None = None) -> str:
    ...
    if not url:
        raise RuntimeError(
            "No encontré URL Postgres/Supabase. "
            "Podés pasarla con --db-url o poner SUPABASE_DATABASE_URL / DIRECT_URL / DATABASE_URL en .env.local."
        )

    if url.endswith(".db"):
        raise RuntimeError("Recibí un archivo .db, pero este script sube cuotas a Postgres/Supabase.")
