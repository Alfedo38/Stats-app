...
export default function WNBATeamMatesPanel({
  ...
  const seasonQuery = `season=${encodeURIComponent(season)}&season_type=${encodeURIComponent(seasonType)}`;
  ...
  const filteredPlayers = useMemo(() => {
    ...
    return (players || [])
      .map((p) => ({
        ...p,
        displayName: p.full_name || p.player_name || `Jugadora ${p.id}`,
      }))
          ...
          filteredPlayers.map((player) => {
            ...
            return (
              <Link
                ...
                href={`/wnba/players/${player.id}?${seasonQuery}`}
