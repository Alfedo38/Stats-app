...
export default function TeamsHub() {
  // Curaduría de los 3 mejores equipos por región principal
  const regions = [
    {
      name: 'LCK (Corea)',
      color: 'text-blue-500',
    ...
    <main className="min-h-screen bg-black text-white p-4 md:p-8 pb-20">
      <div className="max-w-6xl mx-auto space-y-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {regions.map((region) => (
            <div key={region.name} className="bg-[#0a0a0a] border border-[#1a1a1a] rounded-3xl p-6">
              <h2 className={`text-xl font-black uppercase tracking-tighter mb-6 flex items-center gap-2 ${region.color}`}>
              ...
              <div className="space-y-3">
                {region.teams.map((team) => (
                  <Link
                    ...
                    className={`flex items-center justify-between p-4 rounded-xl border border-[#1a1a1a] transition-all group ${region.bgHover} hover:border-gray-700`}
                  >
                    <span className="font-black italic text-lg text-gray-200 group-hover:text-white uppercase tracking-tight">
                      {team.name}
                    </span>
