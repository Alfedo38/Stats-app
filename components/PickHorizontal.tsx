...
export default function PickHorizontal({ play }: { play: any }) {
  return (
    <div className="flex flex-col xl:flex-row bg-[#0a0a0a] border border-[#222] rounded-xl overflow-hidden hover:border-[#444] transition-colors">
      
      {/* 🟢 LADO IZQUIERDO: Info y Stats (40% del ancho en pantallas grandes) */}
      <div className="w-full xl:w-[40%] p-4 border-b xl:border-b-0 xl:border-r border-[#222] flex flex-col justify-between gap-4">
        ...
        <div className="grid grid-cols-3 divide-x divide-[#333] border-t border-[#333] pt-3">
          <div className="text-center">
            <p className="text-[9px] text-gray-500 uppercase tracking-widest mb-1">Proy IA</p>
            <p className="text-white font-bold text-sm">{play.proj}</p>
          </div>
          <div className="text-center">
          ...
          <div className="text-center">
            ...
            <p className="text-white font-bold text-sm">{play.hit_rate ? play.hit_rate.split(' ')[0] : '0/5'}</p>
          </div>
        </div>
      </div>

      {/* 🟢 LADO DERECHO: Descripción de la IA siempre visible (60% del ancho) */}
      <div className="w-full xl:w-[60%] p-4 flex flex-col justify-center bg-[#111]">
