...
export default function LiveGamesCarousel({ liveGames }: { liveGames: any[] }) {
  ...
  if (liveGames.length === 0) {
    return <p className="text-[#666] text-xs font-bold uppercase tracking-widest px-2">No games today.</p>;
  }
  ...
  return (
    <div className="relative group">
        ...
        {liveGames.map((game: any) => (
          <div key={game.id} className="snap-center md:snap-start shrink-0 w-[220px] bg-[#0a0a0a] border border-[#1a1a1a] rounded-[1.5rem] p-4 flex flex-col gap-4 shadow-lg hover:border-[#333] transition-colors relative">
            ...
            <div className="flex justify-between items-center border-b border-[#1a1a1a] pb-2">
              <span className={`text-[9px] font-black uppercase tracking-widest ${game.status === 'in' ? 'text-red-500' : 'text-[#666]'}`}>
                {game.detail}
                {/* Si hay TV, la mostramos sutilmente al lado */}
                {game.tv && <span className="text-[#444] ml-1 opacity-80"> • {game.tv}</span>}
              </span>
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="flex flex-col">
                    <span className="font-black text-xs text-[#ccc] leading-none">{game.away.abbr}</span>
                    {/* Récord del equipo abajo del nombre (Ej: 35-20) */}
                    {game.away.record && <span className="text-[7px] text-[#555] font-black mt-0.5">{game.away.record}</span>}
                  </div>
                </div>
                <span className={`font-black text-lg tabular-nums ${game.away.score > game.home.score ? 'text-white' : 'text-[#666]'}`}>
                  {game.away.score || '-'}
                </span>
              </div>
              ...
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="flex flex-col">
                    <span className="font-black text-xs text-[#ccc] leading-none">{game.home.abbr}</span>
                    {/* Récord del equipo */}
                    {game.home.record && <span className="text-[7px] text-[#555] font-black mt-0.5">{game.home.record}</span>}
                  </div>
                </div>
                <span className={`font-black text-lg tabular-nums ${game.home.score > game.away.score ? 'text-white' : 'text-[#666]'}`}>
                  {game.home.score || '-'}
                </span>
              </div>
            </div>
            ...
            {game.odds && game.odds.trim() !== "" && (
              <div className="mt-1 pt-2 border-t border-[#1a1a1a] text-center">
      ...
      <div className="absolute -top-10 right-2 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
        <button 
          onClick={() => scroll('left')}
          className="bg-[#111] hover:bg-orange-500/20 text-[#888] hover:text-orange-500 border border-[#222] hover:border-orange-500 p-2 rounded-xl transition-all shadow-lg"
        >
          <ChevronLeft size={16} />
        </button>
        <button 
