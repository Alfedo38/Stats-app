...
from nba_incremental_store import SaveReport, upsert_logs, quote
...
quote(TABLE_NAME)  # Validate before interpolating the configured table name.

TIMEZONE = os.getenv("TIMEZONE", "America/Argentina/Buenos_Aires")
LEAGUE_ID = os.getenv("NBA_LEAGUE_ID", "00")
SEASON = os.getenv("NBA_SEASON", "2026-27")
...
SEASON_TYPES = [
    x.strip() for x in os.getenv("NBA_SEASON_TYPES", "Regular Season,Playoffs").split(",")
    if x.strip()
]
...
# Missing metrics stay NULL; the incremental writer preserves existing values.

# Refresca solo la ventana afectada en public.player_page_game_fact_cache.
# Esto es clave porque la web lee la cache, no la vista pesada.
REFRESH_PLAYER_PAGE_CACHE = os.getenv("REFRESH_PLAYER_PAGE_CACHE", "1") == "1"
...
def coalesce_alias(out: pd.DataFrame, target: str, candidates: list[str]) -> pd.DataFrame:
    ...
    for c in existing_candidates:
        if c == target:
            continue
        out[target] = out[target].astype(object).where(out[target].notna(), out[c].astype(object))
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if "resultSet" in msg or "resultSets" in msg:
                raise RuntimeError(f'Respuesta NBA sin dataset: {measure_type} / {season_type}') from e

            if attempt == MAX_RETRIES:
                raise RuntimeError(f'Falló la descarga NBA: {measure_type} / {season_type}') from e
...
def fetch_tracking_for_game(game_id: str) -> pd.DataFrame:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            msg = str(e)
            print(f"      X tracking game={gid} intento {attempt}/{MAX_RETRIES}: {msg}")

            # En varios partidos NBA Stats devuelve payload nulo en BoxScorePlayerTrackV3.
            # No conviene gastar 5 reintentos si el endpoint no trae tracking para ese game_id.
            if "NoneType" in msg and "get" in msg:
                print(f"      ⚠️ tracking {gid}: endpoint sin datos. Se guarda el boxscore base sin tracking.")
                return pd.DataFrame()
...
def prepare_for_db(df: pd.DataFrame, valid_cols: list[str]) -> pd.DataFrame:
    out = apply_boxscore_aliases(df.copy())
    # Unknown metrics stay NULL. Real zeros remain zeros.
    if "game_date" in out.columns:
        out["game_date"] = pd.to_datetime(out["game_date"], errors="raise").dt.strftime("%Y-%m-%d")
    keep = [c for c in out.columns if c in valid_cols and c != "id"]
    return out[keep].loc[:, lambda frame: ~frame.columns.duplicated()].copy()


def save_to_db(df: pd.DataFrame) -> SaveReport:
    if df.empty:
    ...
    records = final_df.astype(object).where(pd.notna(final_df), None).to_dict("records")
    with engine.begin() as conn:
        conn.execute(text("SET LOCAL lock_timeout = '5s'"))
        conn.execute(text("SET LOCAL statement_timeout = '120s'"))
        report = upsert_logs(conn, records, START_DATE, END_DATE, TABLE_NAME)
    print(f"💾 Recibidas: {report.received}; insertadas/actualizadas: {report.changed}; sin cambios: {report.unchanged}")
    return report


# =========================================================
# CACHE REFRESH PARA LA WEB
# =========================================================
def refresh_player_page_cache_window(start_date: str, end_date: str) -> None:
    if not REFRESH_PLAYER_PAGE_CACHE:
        print("⏭️ REFRESH_PLAYER_PAGE_CACHE=0, no refresco player_page_game_fact_cache.")
        return

    try:
        print(f"🧊 Refrescando player_page_game_fact_cache: {start_date} a {end_date}...")
        from refresh_player_page_cache_safe import main as sync_player_cache
        sync_player_cache(['--start', start_date, '--end', end_date], engine=engine)
        print("✅ Cache player_page_game_fact_cache refrescada para la ventana.")
    except Exception as e:
        raise RuntimeError("Los datos pueden estar guardados, pero falló el refresco de caché. Reintentá con --refresh-cache-only para ese rango.") from e

# =========================================================
# MAIN
# =========================================================
def main() -> None:
    parser = argparse.ArgumentParser(description="Actualización NBA incremental por jugador y partido")
    parser.add_argument('--refresh-cache-only', action='store_true', help='Reintenta solo caché, sin consultar NBA ni guardar estadísticas')
    ...
    if args.refresh_cache_only:
        if not REFRESH_PLAYER_PAGE_CACHE:
            raise ValueError('Para reintentar caché usá REFRESH_PLAYER_PAGE_CACHE=1.')
        refresh_player_page_cache_window(START_DATE, END_DATE)
        return
    print("===========================================================")
    print("🔄 DAILY UPDATE INCREMENTAL - PLAYER GAME LOGS")
    print("===========================================================")
    ...
    report = save_to_db(df)
    for day in report.dates:
        refresh_player_page_cache_window(day.isoformat(), day.isoformat())
    if not report.changed:
        print('⏭️ No cambiaron estadísticas: no se reescribió la tabla destino ni se refrescó su caché.')
    print(f"✅ Proceso terminado. Filas insertadas/actualizadas: {report.changed}")
