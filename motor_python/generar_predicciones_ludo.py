...
import numpy as np
import pandas as pd
from ludo_v35_features import add_v35_prediction_features
...
def load_props(engine) -> pd.DataFrame:
    print("📥 Cargando líneas de apuestas...")
    query_props = text(f"SELECT * FROM {VIEW_PROPS};")
    df_props = pd.read_sql(query_props, engine)

    if df_props.empty:
        raise RuntimeError(f"La vista {VIEW_PROPS} no devolvió filas.")

    print("🧠 Conectando el cerebro estadístico (Promedios L5, L10)...")
    # Buscamos la tabla que encontramos recién en tu base de datos
    query_feats = text("SELECT * FROM public.v_ludo_player_recent_features_v2;")
    df_feats = pd.read_sql(query_feats, engine)

    if not df_feats.empty:
        # Aseguramos que los IDs sean números para que el cruce no falle
        df_props['player_id'] = pd.to_numeric(df_props['player_id'], errors='coerce')
        df_feats['player_id'] = pd.to_numeric(df_feats['player_id'], errors='coerce')

        # Nos quedamos con la estadística más reciente de cada jugador
        if "game_date" in df_feats.columns:
            df_feats = df_feats.sort_values("game_date", ascending=False)
        df_feats = df_feats.drop_duplicates(subset=["player_id"])

        # Evitamos mezclar columnas que ya tenemos (nombres, equipos, etc)
        cols_to_drop = [c for c in df_feats.columns if c in df_props.columns and c != "player_id"]
        df_feats = df_feats.drop(columns=cols_to_drop)

        # ¡LA MAGIA! Pegamos las cuotas con el historial del jugador
        df_final = pd.merge(df_props, df_feats, on="player_id", how="left")
        print(f"🔗 Match exitoso: Características inyectadas en las líneas.")
        return df_final
    else:
        print("⚠️ ADVERTENCIA: No se encontraron features. El modelo predecirá a ciegas.")
        return df_props
...
def main():
    ...
    df = add_v35_prediction_features(df, engine, registry)
