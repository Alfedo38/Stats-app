...
import { Target, Zap, Trophy, Flame, ChevronRight, ShieldCheck } from 'lucide-react';
...
export default async function OnFirePage() {
  ...
  const sections = [
    { title: 'Líderes en Puntos', key: 'puntos', statKey: 'pts_avg', icon: <Target className="text-orange-500" />, label: 'PTS', color: 'border-orange-500/20' },
    { title: 'Líderes en Rebotes', key: 'rebotes', statKey: 'reb_avg', icon: <Zap className="text-blue-500" />, label: 'REB', color: 'border-blue-500/20' },
    { title: 'Líderes en Asistencias', key: 'asistencias', statKey: 'ast_avg', icon: <Trophy className="text-green-500" />, label: 'AST', color: 'border-green-500/20' },
  ];
        ...
        {sections.map((section) => (
          <section key={section.key} className="space-y-6">
            <div className="flex items-center gap-3">
              {section.icon}
              <h2 className="text-xs font-black uppercase tracking-[0.2em] text-[#666]">{section.title}</h2>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {(data as any)[section.key]?.map((player: any) => (
                <Link 
                ...
                >
                  <div className="flex justify-between items-start relative z-10 mb-8">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-2xl bg-black border border-[#222] flex items-center justify-center font-black text-[#666] group-hover:text-white transition-colors">
                        {player.team_abbr}
                      </div>
                      <div>
                        <h3 className="text-xl font-black uppercase tracking-tighter leading-none">{player.full_name}</h3>
                        <p className="text-[9px] text-[#444] font-bold uppercase mt-1 tracking-widest">PostgreSQL Stat</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-black/40 border border-[#111] p-6 rounded-3xl text-center relative z-10">
                      <p className="text-4xl font-black text-white">
                        {player[section.statKey]?.toFixed(1) || '0.0'}
                      </p>
