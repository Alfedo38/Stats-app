...
export default function EVCarousel({ evPlays }: { evPlays: any[] }) {
  ...
  return (
    <div className="relative group">
          ...
          return (
            ...
            >
              <div className={`
                ...
                border ${isTopPlay ? 'border-[#10b981]/50' : 'border-[#1a1a1a]'}
              ...
              `}>
                ...
                <div className="flex justify-between items-end">
                  <div className="flex flex-col">
                    ...
                    <span className="text-white/80 font-bold text-xs tabular-nums">{avgLast10.toFixed(1)}</span>
                  </div>
                  <div className="flex flex-col items-end">
      ...
      <button
        onClick={() => scroll('left')}
        className="absolute -left-2 top-1/2 -translate-y-1/2 bg-black/60 backdrop-blur-md border border-white/10 p-2 rounded-full text-white opacity-0 group-hover:opacity-100 transition-all z-30 hidden md:flex"
      >
        <ChevronLeft size={16} />
      </button>
      <button
        onClick={() => scroll('right')}
        className="absolute -right-2 top-1/2 -translate-y-1/2 bg-black/60 backdrop-blur-md border border-white/10 p-2 rounded-full text-white opacity-0 group-hover:opacity-100 transition-all z-30 hidden md:flex"
      >
