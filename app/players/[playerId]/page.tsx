...
function clientPlain(value: any): any {
...
async function soft<T>(label: string, promise: Promise<T>, fallback: T, ms = 900): Promise<T> {
  let timer: any;

  const timeout = new Promise<T>((resolve) => {
    timer = setTimeout(() => {
      console.warn(`[player-page] ${label} timeout ${ms}ms; usando fallback`);
      resolve(fallback);
    }, ms);
  });
  ...
  try {
    const result = await Promise.race([
      promise.catch((error) => {
        console.warn(`[player-page] ${label} error:`, error?.message || error);
        return fallback;
      }),
      timeout,
    ]);
...
export default async function PlayerPage(props: any) {
  try {
    ...
    const playerDataFastStart = Date.now();
    let data = await getPlayerDataFast(playerId);
    console.log(`[player-page] getPlayerDataFast ${playerId}: ${Date.now() - playerDataFastStart}ms`);

    if (!data?.player || !Array.isArray(data?.stats) || data.stats.length === 0) {
    ...
    const rosterTeams = normalizedTeamAbbr ? [normalizedTeamAbbr] : [];

    // Render inicial liviano:
    // - No bloqueamos HTML con schedule/scoreboard ESPN.
    // - No bloqueamos HTML con injury context; PlayerChartContainer ya lo carga por API cliente.
    // ── Render inicial turbo ─────────────────────────────────────────────────
    // Compañeros queda lazy/no bloqueante por ahora.
    // Los compañeros se cargan desde el cliente para no bloquear el HTML inicial.
    const teammates: any[] = [];
    ...
    const clientStakeOdds = JSON.parse(JSON.stringify(clientPlain(stakeOdds))) as any[];
    const clientTeammates = JSON.parse(JSON.stringify(clientPlain(teammates))) as any[];
    ...
    return (
      <main className="min-h-screen bg-[var(--bg)] text-[var(--text)] font-sans pb-20">
        ...
        <div className="p-4 md:p-6 2xl:p-8 max-w-[1700px] mx-auto space-y-4">
          ...
          <PlayerPageContent
            key={`${playerId}-${requestedStat}`}
            stats={clientStats}
            navStats={NAV_STATS}
            playerName={playerName}
            initialStat={requestedStat}
            stakeOdds={clientStakeOdds}
            teammates={clientTeammates}
